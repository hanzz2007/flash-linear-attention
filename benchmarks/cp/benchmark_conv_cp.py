...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--kind', choices=('kernel', 'cp'), default='cp')
...
def _run_once(args: argparse.Namespace, inputs, do: torch.Tensor, cp_context) -> torch.Tensor:
...
def _measure(args: argparse.Namespace, inputs, do: torch.Tensor, cp_context, worker_device: torch.device):
...
def main() -> None:
    args = _parse_args()
    os.environ['FLA_ASCEND_CONV_PRECISION'] = args.precision
    os.environ['FLA_CP_CONV_COMM'] = args.comm
